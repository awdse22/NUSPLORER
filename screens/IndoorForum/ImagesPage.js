    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.uploadPhotoContainer}>
        <AddDataButton
          label="Upload Photo"
          onPress={() =>
            navigation.navigate('Upload Image', { roomId: roomId, dataType: dataType })
          }
        />
      </View>
      <ScrollView>
        <View style={styles.imageDisplayWrapper}>
          {images.length == 0 && (
            <Text style={styles.noInformation}>There are no images of {dataType}</Text>
          )}
          {images.map((image) => {
            const imageData = {
