            ...
            renderItem={(item) => {
              return (
                <View style={styles.dropdown.itemContainer}>
                  <Text style={styles.dropdown.itemText}>{item.label}</Text>
                  {item.value == reason && <AntDesign name="check" size={22} color="black" />}
                </View>
              );
            }}
          />
          {errorVisible && (
            <Text style={{ color: 'red', fontSize: 16, fontWeight: 'bold' }}>{errorMessage}</Text>
          )}
          <View style={styles.subContainer}>
